...
**構造的に解決できないもの（記録のみ）**
- ~~添付が無い人材が18.4%~~ **この数字は誇張だった（2026-08-12 検証）**。
  旧監査は resume_url/box_url 列しか見ておらず、真に入力が無いのは **6.5%（56/859件）**。
  残りは「添付あり・経歴書扱いされず」11.2%（割当/パースで直せる）と
  「Driveリンクのみ・未取込」2.3% に分解される。検証SQL: `scripts/sql/audit_no_resume_claim*.sql`
- カタログ型（スキル一覧のみで期間が無い）経歴書は引き続き構造的に不可
