...
### ① 到達レベルが SQL 側の順位に効いていない（原因未特定・**本番未投入**）

`fetch_candidates_for_project` にレベルを渡すパッチを書いて **15回試走したが順位が1つも動かない**。
役割の重みを1000倍（150pt差）にしても動かない。一方、役割そのものを変えると
1位→381位 に動くので測定器は生きている。

確認済み: パッチは当たる（115行にレベル列・202行に3引数呼び出し）／`pre` は素の
`candidates` を読む／RPC外でレベルが読める／`role_affinity('PM','PMO','A')`=0.45。

**原因が分からないものは本番に入れない**方針で保留。
パッチ本体は `supabase/migrations/PENDING_role_level_rpc.sql.txt`。
