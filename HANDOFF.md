...
- **AI が読めない経歴書の残り4件（西暦表記が無い型）**。8/14 に記入フォーム型を
  対応して 11件中7件は解決した。残るのは **AN / TK / MK / I.Y** で、いずれも
  `年セル=0`（全セルは 108〜928 あるので中身はある）。和暦（`R5.11`）か
  期間長（`2年0ヶ月`）だけで書かれているとみられる。**未調査**。
  - 調べ方: `node scripts/audit_llm_unreadable_resumes.mjs`（分類）→
    `node scripts/probe_year_cell_layout.mjs <candidate_id>`（実物の並び）
  - **実物の並びを数えてからフィクスチャを作ること**（想定で作って事故った）
  - 期間長型は §4 の PDF 側の課題と同じ性質。「経験年数 26年10ヶ月」のような
    **合計行を案件として加算しない**除外規則とセットで設計する
...
- **ワーカーの `claude -p` がプロジェクト背景を無駄に読んでいる**（8/14 実測）。
  cwd がプロジェクトのため CLAUDE.md 等が毎回載り、**1回あたり約7,000トークン**
  余分（プロジェクト内 15,953 / 無関係なディレクトリ 9,006）。抽出プロンプトは
  自己完結なので不要。`caller.mjs` の spawn に cwd を渡せば済む（1行）
