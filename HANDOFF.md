# 引き継ぎ（2026-08-12 夕方時点）

## 0. 次セッションの最初にやること

### ① スキル一致判定が緩すぎる（最優先・ユーザー了承済みで別タスク化）

マッチングの必須スキル判定が部分一致で、**別物のスキルが一致してしまう**。

RPC の条件（`fetch_candidates_for_project`）:
```sql
lower(s) LIKE '%' || q || '%' OR q LIKE '%' || lower(s) || '%'
-- q=必須スキル(小文字) / s=候補者スキル
```
...
| ケース | 人数 |
|---|---|
| **JavaScript しか持たないのに「Java」要件に一致** | **245人** |
| SQL を持たないのに MySQL 等で「SQL」要件に一致 | 335人 |
| 「テスト」要件に一致（単体テスト等を含む） | 1,481人（74%） |

「テスト」で全体の74%が一致するため、**ほぼ全員が必須スキル満点近くになる**。
今日入れた重み付け（下記②）が順位をほとんど動かさなかったのはこれが原因。
重み機構自体は正常（極端な重みを与えると順位が総入れ替えになることを確認済み）。

**直し方の案**: `skill_master` の別名で正規化してから比較する（正規化キー2,226件が既にある）。
- `Java8` `Java SE` → 別名で `Java` に寄る → **一致**
- `JavaScript` → 正式名が別 → **不一致**
- マスタに無い表記は従来どおり空白・大小文字を落とした完全一致

**ユーザー判断が要る点**: `MySQL` の人を `SQL` 案件で落としてよいか。
ユーザーコメント「SQLマスターみたいなイメージがあるし」＝ MySQL 等も SQL 経験として
見たい含み。落としたくないなら `skill_master` の `SQL` の別名に MySQL 等を足せば
データ側で調整できる（コード変更不要）。**厳格化の前に、何人が候補から外れるかを測ること。**

検証に使ったSQL: `scripts/sql/verify_project_skill_weights.sql`

### ② 人材の skillYears 一括再解析（中断したまま）
