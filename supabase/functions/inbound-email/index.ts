...
/** 自己PR に紛れ込んだ「プロフィール欄の見出し」。3つ以上並んでいたら自己PRではない */
const PROFILE_LABEL_RE =
  /【?\s*(?:氏\s*名|年\s*齢|単\s*[価金]|最寄|所\s*属|工\s*程|開発言語|稼\s*働|日本語|IT経験|実務経験|スキル)\s*】?\s*[:：]?/g

/**
 * 自己PR として保存してよい文章か整える。
 *
 * 実測（2026-09-21・ローカル控えで自己PRを持つ368人）:
 *   本来の自己PR              289人 (78.5%) 中央値189字
 *   プロフィール欄の丸写し      30人 (8.2%)  ← 自己PRではない
 *   URLが混ざっている           49人 (13.3%) ← スキルシートのリンク等
 *
 * 丸写しの例:
 *   「【氏 名】楊F(ヨウ) 【年 齢】40歳（男） 中国籍 【日本語】一級流暢 【経 験】17年
 *     【単 金】65万円 【工 程】基本設計、詳細設計、製造…」
 *   これは本文そのもので、氏名も年齢も単価も既に構造化された列に入っている。
 *   画面に二重で出るうえ長いだけなので捨てる。
 */
function sanitizeSelfPR(text: string | null): string | null {
  if (!text) return null
  // URL は自己PRの中身ではない（スキルシートへのリンクが本文ごと拾われる）
  let s = text.replace(/https?:\/\/\S+/g, '').replace(/[ \t　]{2,}/g, ' ').trim()
  // 区切り線だけの行を落とす
  s = s.replace(/^[-―ー─=＝_]{3,}$/gm, '').replace(/\n{3,}/g, '\n\n').trim()
  if (!s) return null
  // プロフィール欄の見出しが3つ以上 → 本文の丸写しなので自己PRとして扱わない
  const labels = (s.match(PROFILE_LABEL_RE) ?? []).length
  if (labels >= 3) return null
  // 整えた結果が短すぎるものは情報がない
...
function extractSelfPR(body: string, _attachText: string): string | null {
  ...
  return sanitizeSelfPR(extractSectionsByLabels(decodeHtmlEntities(body), [
...
/**
 * 経歴書・スキルシートの「欄の名前」を人名として採らない。
 *
 * ⚠ 語を足すときは **人名として実在しうるか** を必ず考えること。
 *   完全一致（空白とコロンを除いて ^…$）なので、「学」「望」のような
 *   1字の語を入れると本物の人名を消す。1字の語は入れない。
 */
function isLabelWordName(v: string | null | undefined): boolean {
  const s = String(v ?? '').replace(/[\s　:：]+/g, '')
  if (!s) return true
  const WORDS = [
    // 従来からあるもの（2026-09-09 prod 実測で人材になっていた見出し語を含む）
    '氏名', 'お名前', '名前', '候補者名', 'フルネーム',
    'ふりがな', 'フリガナ', 'よみがな', 'ヨミガナ', 'カナ',
    '生年月日', '年齢', '性別', '住所', '現住所', '居住地',
    '最寄駅', '最寄り駅', '沿線', '国籍',
    '経験年数', '経験', 'スキル', '保有スキル',
    '希望単価', '単価', '希望', '備考', '補足', 'コメント',
    '専攻学科', '学歴', '最終学歴', '卒業', '資格', '保有資格',
    '所属', '会社名', '企業名', '担当', '役職', '部署',
    '自己PR', 'PR', '稼働', '稼働時期', '参画時期',
    '連絡先', '電話番号', 'メールアドレス',
    'No', 'NO', 'ナンバー', '番号', '項目', '区分', '状況', 'ステータス', '新規',

    // 2026-09-18 追加。スキルシートの見出し語が氏名になっていたのを画面で発見した。
    //   実害: (株)Dearism の1通で氏名が「専門分野」。
    //   本文には ≪N.M (36歳) 男性≫ とあり extractNameFallback は "N.M" を返せていたのに、
    //   添付から拾った regexFields.name が先に採用され、見出し語が本人を押しのけていた。
    //   ここで弾けば本文のイニシャルに落ちる。
    '専門分野', '得意分野', '対応分野', '専門分野等', '専門',
    '職種', '業種', '業務', '業務内容',
    '工程', '対応工程', '担当工程', '作業工程', '担当業務',
    '開発言語', '使用言語', '言語',
    '保有技術', '技術', '開発環境', '使用環境', '環境',
    'ツール', 'フレームワーク',
    '職務経歴', '経歴', '履歴', '実績', '概要', '要約', 'サマリ', 'サマリー',
    '特記事項', '特記', '事項', '種別', '属性',
    '内容', '詳細', '情報', '一覧', 'その他', '以上',
  ]
...
function extractCandidateFieldsRegex(
  ...
  let nearestStation = extractFieldTwoPhase(
    ...
    v => {
      ...
      // 駅名のうしろに注記が続く形（2026-09-17 実測）。
      //   「最 寄 駅：京王線　八幡山駅　リモート併用案件希望　※通勤30分以内であれば常駐可」
      // 従来は `[駅線]$` を要求していたため、注記が付くと丸ごと捨てていた。
      // 途中に駅トークンがあれば通し、駅名の切り出しは parseNearestStation に任せる
      // （実測でこの関数は注記・括弧・徒歩分数を正しく落とす）。
      if (/[^\s　（(]{1,10}駅(?:[\s　（(]|$)/.test(c)) return true
      return /[駅線]$/.test(c) || (c.length <= 10 && /[^\x00-\x7F]/.test(c))
    },
    // ⚠ 30 だと上の実データ（35文字）が長さで先に落ちる。
    //   check() は maxLen を validate より先に見るため、ここを伸ばさないと validate に届かない。
    60,
    2,
  )
  ...
  let availableFrom = extractFieldTwoPhase(
    ['参画開始可能日','参画可能時期','参画可能','稼働開始月','稼働開始','稼働可能時期','稼働可能','稼働時期','開始可能日','稼動時期','稼働日','稼動日','稼動開始','稼動可能','稼働','稼動','参画時期','参画開始','就業開始','就業時期','就業可能時期'],
  ...
  while ((m = PRE_RE.exec(sigArea)) !== null) {
    // 直前が社名構成文字なら「◯◯株式会社」（後株）の一部であり前株ではない
...
function scoreProseRoles(
  ...
  /** 根拠が弱い役割の理由。'工程'=工程の列挙の中 / '作業'=作業語だけ。営業に見せる印 */
  roleEvidence: Record<string, string>
} {
  const ROLE_DEFS: Array<{ re: RegExp; label: string; weak?: RegExp }> = [
    ...
    // プロダクトオーナーは同じマス（Scrum Guide: 製品価値の最大化に責任を持つ）なので
    // 別ラベルにせずここに寄せる。裸の `PO` は誤爆が多いので採らない。
    { re: /(?<![A-Za-z])PdM(?![A-Za-z])|プロダクト[　 ]?(?:マネージャー|マネジャー|オーナー)/, label: 'プロダクトマネージャー' },
    ...
    // ⚠ `PL/SQL`・`PL/pgSQL` を PL（役割）と読まないこと（2026-09-15 実測）。
    //   ローカル控え7,325通の監査で、**PLが付いた1,452通のうち248通(17.1%)が
    //   スキル一覧の PL/SQL だけを根拠**にしていた。どれも言語の列挙で役割ではない:
    //     「言語：C#.Net，Java，VB.NET，C言語，PL/SQL，JS，HTML，CSS」
    //     「ツール：Eclipse、A5MK2、VSCode、Git、PL/SQL Developer」
    ...
    // Web制作・コンテンツの職種（2026-09-21 追加）。
    // きっかけ: 【ONメンバー】【ディレクション、要件定義、ワイヤーフレーム作成、進行管理、
    //   バナー制作、顧客折衝】の人に役割が1つも付いていなかった（ユーザー指摘）。
    // 実測（言語スキル0本＝本当の非エンジニアに絞った568人中）:
    //   ディレクター 76人 単価78.5万 / マーケター 31人 単価90万
    //   （比較: 役割が付いている人の中央値は70万）。単価で分かれるので足す根拠がある。
    //   デザイナーは45人・単価70万で全体と同じだったため足さない
    //   （CLAUDE.md「分かれない役割に印を付けない。意味のない印は害になる」）。
    // ⚠ 「進行管理」はエンジニアも普通に書く。最初その語で数えて437人・単価70万と出て、
    //   役割持ちと1円も変わらなかった。**名乗っている形だけ**を拾い、作業語は weak に置く。
    { re: /ディレクター(?!ズ)|ディレクション/, label: 'ディレクター',
      weak: /(?:進行|制作)[　 ]?管理/ },
    { re: /マーケター|マーケティング[　 ]?(?:担当|責任者|マネージャー)|(?:Web|WEB|デジタル)[　 ]?マーケティング|広告[　 ]?運用/, label: 'マーケター',
      weak: /広告[　 ]?運用/ },
    ...
    // モバイルが丸ごと抜けていた（2026-09-15 ユーザー指摘「iOSエンジニアって書いてるやん」）。
    // 「iOSエンジニアとしてSwift・SwiftUIを中心に長年の開発経験」と名乗っている人の役割が、
    // 末尾の「リーダー経験もあり」だけを拾った プロジェクトリーダー 1件になっていた。
    // 実測（prod 3,225人・2026-09-15）: モバイル開発を職種として書いている101人のうち
    // 33人が製品系の役割ゼロ。職種名を名乗る8人のうち7人が PL/PM/PMO 扱いだった。
    // ⚠ Swift/Kotlin/Xcode 等の**技術名からは推定しない**。技術はスキル側で持つ
    //   （裸の一般語から役割を起こさない・CLAUDE.md「役割・業界の抽出」と同じ理由）。
    // ⚠ 「アプリエンジニア」単独は Web アプリと紛れるので英字・ウェブの前置きガードを付ける。
    { re: /(?:iOS|Android|モバイル|スマ(?:ート)?(?:フォン|ホ))[　 ]?(?:アプリ(?:ケーション)?)?[　 ]?(?:エンジニア|開発者|開発)|(?<![A-Za-z]|ウェブ|ブラウザ)アプリ[　 ]?エンジニア/, label: 'モバイルアプリエンジニア' },
    ...
    // SRE（2026-09-16 追加）。運用保守と**同じ対象（サービス）だが権限が1段上**。
    // Google SRE のエラーバジェット方針は「信頼性が枯れたらリリースを止める」判断権を持つ。
    // 決裁（予算・契約）は持たないので L3（統率）。控えで66通、いま運用保守23/インフラ18に散っていた。
    { re: /(?<![A-Za-z])SRE(?![A-Za-z])|サイト[　 ]?リライアビリティ/, label: 'SRE' },
    // 社内SE・情シス（2026-09-16 追加）。**作用対象が「事業（自社の業務・IT資産）」**で、
    // 受託の製品を作る SE とも、他社サービスを受ける ヘルプデスク とも違う。
    // 控えで340通。いま システムエンジニア153 / ヘルプデスク142 に割れていた。
    // 「情報システムエンジニア」（職種名）を巻き込まないよう部署語だけを見る。
    { re: /社内[　 ]?SE(?![A-Za-z])|情シス|情報[　 ]?システム[　 ]?(?:部|室|課|担当|部門)/, label: '社内SE' },
  ...
  const NEGATION_AFTER =
    ...
    // 「経験はありません」だけを見ていたため「経験がない」を取りこぼしていた
...
function extractFromProse(bodyText: string, attachText: string): {
  ...
  roleLevels: Record<string, string>
  /** 根拠が弱い役割の印。'工程'=工程の列挙の中だけ / '作業'=作業語だけ。営業に理由を見せる */
  roleEvidence: Record<string, string>
  ...
  if (langColIdx >= 0) {
    ...
    if (Object.keys(skillMonths).length > 0) {
      ...
      if (projectPeriods.length > 0) {
        ...
        const starts = projectPeriods.map(p => p.startYM ?? parseYM(p.start)).filter((v): v is number => v !== null)
        const ends   = projectPeriods.map(p => p.endYM   ?? parseYM(p.end)).filter((v): v is number => v !== null)
    ...
    if (type === 'candidate' || type === 'human') {
          ...
          try {
            ...
            const blockPayload = {
              ...
              raw_profile: {
                ...
                _roleEvidence: Object.keys(blockProseFields.roleEvidence).length > 0 ? blockProseFields.roleEvidence : undefined,
      ...
      const dbPayload = {
        ...
        raw_profile: {
          ...
          _roleEvidence: Object.keys(proseFields.roleEvidence).length > 0 ? proseFields.roleEvidence : undefined,
