...
function filterSkillYears(sy: Record<string, number>): Record<string, number> {
  ...
  const WORK_STYLE_RE = /リモート(?!デスクトップ)|常駐|出社|通勤|^在宅|応相談|^\d{2,3}-\d{2,3}$/
  ...
  const PHASE_LABEL_RE = /^(要件|定義|要件定義|基本設計|詳細設計|外部設計|内部設計|製造|コーディング|単体試験|結合試験|総合試験|受入試験|運用保守|期間|稼働月数|担当範囲|体制|担当|作業内容|業務内容|担当業務|経験技術|作業概要|能力指標|能力判断)$/
