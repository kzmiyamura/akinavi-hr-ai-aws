...
const ROLE_AXIS: Record<string, { object: string; authority: number }> = {
  ...
  'プロダクトマネージャー':   { object: '製品',     authority: 4 },
  '社内SE':                   { object: '事業',     authority: 2 },
  'SRE':                      { object: 'サービス', authority: 3 },
  ...
  // Web制作・コンテンツ側（2026-09-21 追加）
  //   ディレクター … 作用対象は「成果（制作物）」。要件定義からワイヤーフレーム、
  //                  制作メンバーへの指示出しと進行管理まで担う＝統率(3)。
  //                  プロジェクトリーダーと同じ座標になるが、この2軸は領域を表さない
  //                  設計（エンジニア職も全員 製品×2 で重なる）。制作かシステムかは
  //                  スキル一致側で分かれる。
  //   マーケター   … 作用対象は「事業（売上・集客）」。広告運用や分析を自分で回す＝実行(2)。
  //                  同じ事業でもコンサルタント(3)とは権限が違う。
  'ディレクター':             { object: '成果',     authority: 3 },
  'マーケター':               { object: '事業',     authority: 2 },
}
