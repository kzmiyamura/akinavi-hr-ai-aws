-- #85: 派遣案件フィルターに「正社員のみ」条件を追加
-- p_require_haken=true のとき、派遣免許あり会社 かつ employmentType='正社員'（不明は通過）のみ返す

DROP FUNCTION IF EXISTS fetch_candidates_for_project(text, text[], numeric, numeric, text, text, integer, integer, integer, integer, integer, boolean);
...
BEGIN
  ...
  WITH pre AS (
    ...
    WHERE c.data_env    = p_data_env
      ...
      -- 派遣フィルター: 派遣免許あり会社 かつ employmentType='正社員'（null=不明は通過）
      AND (
        NOT p_require_haken
        OR (
          EXISTS (
          ...
          AND COALESCE(c.raw_profile->>'employmentType', '正社員') = '正社員'
        )
      )
  )
  SELECT c.*
