-- search_candidates / count_search_candidates RPC修正
-- skills カラムが jsonb 型のため array_to_string が使えない問題を修正
-- array_to_string(skills, ' ') → skills::text に変更
...
-- 案件検索も同様に修正（required_skills が jsonb の場合に備え）
...
BEGIN
  FOREACH kw IN ARRAY p_keywords LOOP
    cond := format(
      '(title ILIKE %L OR required_skills::text ILIKE %L OR raw_data::text ILIKE %L)',
      '%' || kw || '%',
      '%' || kw || '%',
      '%' || kw || '%'
    );
