...
- requiredSkills は「必須・必須スキル」に列挙された技術・ツール・言語のみ。「基本設計」「詳細設計」「テスト」「設計」「開発経験」等の工程名・作業内容は含めないでください。
- 本文に明記されていない技術・ツールは推測して追加しないでください（例: 「SQL」と書かれていても「PostgreSQL」「MySQL」を勝手に追加しない）。
- 尚可・歓迎は niceToHaveSkills に入れてください。
