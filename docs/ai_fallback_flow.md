...
画像添付あり OR (Cerebras/Groq キーなし)
  └─► Gemini 2.5 Flash（gemini-2.5-flash）※画像対応
        └─ 添付バイナリ＋本文で解析
...
- Groq の日次トークン上限（100K TPD）はUTC 0:00（JST 9:00）リセット
...
| モデル | 用途 | 無料枠 | コンテキスト |
|---|---|---|---|
| Cerebras `llama3.1-8b` | 関連性チェック・マッチングスコア | 大（実質無制限） | 8K tokens |
| Groq `llama-3.3-70b-versatile` | 候補者/案件構造化抽出 | 100K tokens/日（JST 9:00リセット） | 128K tokens |
| Gemini `gemini-2.5-flash` | フォールバック・画像解析 | 有料（クレジット制） | 1M tokens |
| Gemini `gemini-2.0-flash` | 関連性チェックのフォールバック | 有料 | 1M tokens |

---

## Groq 枯渇時の挙動（JST 0:00〜9:00）
