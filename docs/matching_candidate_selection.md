...
### フルリモート希望 × 常駐案件のペナルティ（コミット `637152f`）

`raw_profile.wantsFullRemote=true`（人材が「フルリモート希望」と明示）かつ案件側に `リモート / remote / 在宅 / フルリモート` の記述がない場合は **合計 30pt にキャップ**。
`breakdown` 末尾に `[フルリモート希望・常駐案件のため30pt上限]` という注記が付き、UI でも理由が確認できる。
これにより「スキル満点でもフルリモート希望者を常駐案件の上位に並べてしまう」問題を防止。

抽出側（`inbound-email`）は `workStyle=フルリモート` を検出したときに `raw_profile.wantsFullRemote=true` を保存する。
...
### AI スコアの扱い（コミット `c62bac2` 以降・現行）

旧版は AI が返したスコアを `ruleScore ± 15pt` にクリップしていたが、現行は **AI に score 変更を禁止** する方針に変更（プロンプトに「`score（整数）: 各候補者の score をそのまま使うこと（変更禁止）`」を明記）。
AI は事実上 `summary` の自然言語化だけを担当し、最終スコアは `calcRuleScore` の結果がそのまま採用される。AI が誤って score を変えて返してきても、表示上は AI スコア（`aiResults[i].score`）が優先採用される実装になっているため、プロンプト遵守がキーとなる。AI 全段失敗時はルールスコアにそのまま代替（`usedModel='rule'`・`summary=''`）。
