...
// DO NOT EDIT — 2026-09-23 に index.ts から生成
...
// ── splitSkillNameCell ──
function splitSkillNameCell(cell){
  const out = []
  const push = (s) => {
...
function extractSkillYearsFromNamedTable(grid){
  const out= {}
...
// ── bareResumeUrlsInRow ──
function bareResumeUrlsInRow(row, rowIndex){
  const out = []
  const seen = new Set()
...
// ── extractNameFallbackRaw ──
function extractNameFallbackRaw(text){
...
// ── isZipAttachment ──
function isZipAttachment(att){
  return ZIP_MIME.includes(att.mimeType) || /\.zip$/i.test(att.name ?? '')
}

// ── planZipEntries ──
function planZipEntries(zipLabel, entries) {
  const picks = []
  const skipped = []
  let total = 0
