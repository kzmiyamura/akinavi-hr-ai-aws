 ...
 * 件名が「人材の紹介」だと明言しているか。
 *
 * 非人材検知（隔離）の歯止めに使う。AI が mailType を誤判定しても、
 * 件名がこの形なら隔離しない。
 * 実害（2026-08-19 発覚）: Haiku が「☆彡【直人材】TypeScript/…」「【弊社社員】Java/…」
 * 「株式会社◯◯ 営業人材一覧」を other と判定し、本物の人材10件が一覧から消えていた。
 *
 * 案件メールにも「人材募集」等が出るため、**紹介する側の言い回し**に限定する。
 */
export function looksLikeCandidateSubject(subject) {
  const s = String(subject ?? '')
  if (!s) return false
  return /(直人材|弊社社員|自社社員|弊社\s*FL|弊社フリーランス|人材一覧|要員一覧|人材情報|要員情報|人材紹介|要員紹介|人材のご紹介|要員のご紹介|エンジニアをご紹介|ご紹介します)/.test(s)
}
