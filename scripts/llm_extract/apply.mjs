...
export function isUsableName(n) {
  ...
  // 経歴書・スキルシートの見出し語（2026-09-18）。
  // 画面で「氏名＝専門分野」の人材が見つかった。しかもその行には
  // 「AIが補正した項目 3件」のバッジが付いていた。直っていなかった理由はこれ:
  //   bodyLooksComplete() が isUsableName('専門分野') を true と見なし、
  //   氏名・単価・会社・年齢・駅が揃っている＝「本文は充足済み」と判断して
  //   **本文LLMを丸ごと省略**していた。name の方針は overwrite なのに、
  //   AI は本文を一度も読んでいなかった。
  // ここで false を返せば本文LLMが走り、AI が氏名を直せる。
  // inbound-email 側の isLabelWordName と揃えること（片方だけ直すと漏れる）。
  if (/^(専門分野|得意分野|対応分野|専門|職種|業種|業務|業務内容|工程|対応工程|担当工程|作業工程|担当業務|開発言語|使用言語|言語|保有技術|技術|開発環境|使用環境|環境|ツール|フレームワーク|職務経歴|経歴|履歴|実績|概要|要約|サマリ|サマリー|特記事項|特記|事項|種別|属性|内容|詳細|情報|一覧|その他|以上|備考|補足|スキル|資格|学歴|所属|役職|部署|希望|単価|年齢|性別|住所|国籍|最寄駅|沿線|稼働)$/.test(s)) return false
