...
const targets = cands.filter((c) => !c.raw_profile?._llm_checked_at &&
  (processedAt.has(c.id) || c.raw_profile?._llm_applied?.at))
const stillPending = cands.filter((c) => !c.raw_profile?._llm_checked_at && !processedAt.has(c.id) && !c.raw_profile?._llm_applied?.at)

console.log(`=== 校正フラグの状態（prod・直近${DAYS}日・${cands.length}件）===`)
console.log(`印あり（正しく表示）:        ${cands.length - targets.length - stillPending.length}件`)
...
if (!APPLY || targets.length === 0) { console.log(`\n（--apply で ${targets.length}件に遡及付与）`); process.exit(0) }

let ok = 0
for (const c of targets) {
  const at = processedAt.get(c.id) ?? c.raw_profile._llm_applied.at
  ...
  if (res.ok) ok++
  else console.log(`  失敗: ${c.name} ${res.status}`)
}
