...
- **Excel skillYears**: 100.0%（Pass:15 Warn:0 Fail:0/15）
- **Body skillYears**: 100.0%（Pass:3 Warn:0 Fail:0/3）
- **メモ**: TMK実メール検証(2026-07-30): ①後株社名+人名の前株誤認ガード(ＷｅａＬｉｖｅ株式会社徳田です→株式会社徳田の実害) ②稼動日/稼働日ラベル対応(即日～取得) ③filterSkillYears正規化4種(全角数字/バージョン断片9i,11g/連続スペース複合キー/セクションラベル：値)

---

## 2026-07-30 イテレーション（自動記録）
- **Excel skillYears**: 100.0%（Pass:15 Warn:0 Fail:0/15）
- **Body skillYears**: 100.0%（Pass:3 Warn:0 Fail:0/3）
...
## 2026-08-10 イテレーション（自動記録）
- **Excel skillYears**: 100.0%（Pass:9 Warn:1 Fail:0/10）
- **Body skillYears**: 100.0%（Pass:3 Warn:0 Fail:0/3）
- **メモ**: スキル一覧表のヘッダーを案件表の技術列と誤認する問題を修正（見出し直後5行に日付があるかで検証・通らなければ従来のフォールバック）。A_S が是正され 8Pass→9Pass

---

## 2026-08-12 イテレーション（自動記録）
- **Excel skillYears**: 100.0%（Pass:9 Warn:1 Fail:0/10）
- **Body skillYears**: 100.0%（Pass:3 Warn:0 Fail:0/3）
- **メモ**: 名簿取りこぼし対応: 行上限15→70(膨張ガード30→70連動)・名簿リンク取得に最低2.5秒予算・添付割当に管理番号マッチ(パス1.5)追加・顧客をlooksLikeRosterNameブロックリストへ
