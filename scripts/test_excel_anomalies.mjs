...
console.log('=== K4. stripInitialSuffix（氏名を3文字に切らない・#128） ===')
{
  ...
  // どちらも営業がその表記で人を指しているので、切ると人を特定できなくなる
  s('K4-6: IT-OGK（ハイフン識別子）は切らない', 'IT-OGK', 'IT-OGK')
  s('K4-7: IC023009（管理番号）は切らない', 'IC023009', 'IC023009')
  s('K4-8: IT-OGK＋年齢性別でも識別子を残す', 'IT-OGK　56歳　男性', 'IT-OGK　56歳　男性')
  // 回帰防止: 数字のあとに更に文字が続くものは従来どおり切る（年齢＋駅名の巻き込み）
  s('K4-9: KM29蕨（年齢＋駅名）はイニシャルのみ', 'KM29蕨', 'KM')
  // 「KM29」は4文字なので、元からある「イニシャル+2文字以内は切らない」規則でそのまま残る。
  // ここで切る必要は無い。数字を含む氏名は下流の isUsableName が弾く（apply.mjs）
  s('K4-10: KM29 は長さ規則でそのまま（isUsableName が弾く）', 'KM29', 'KM29')
