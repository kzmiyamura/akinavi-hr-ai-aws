...
t('G4: 実スキル3件未満なら採用しない（誤発動ガード）',
  [BH,
   ...
   ['', '', 'PHP', '', '']],
  {})
// 「現在」は実行日を基準に数えるので、開始を固定日付にすると時間が経つほど期待値からずれる。
// 2026/01 固定だったため 2026-09 に入って 9ヶ月になり失敗した（2026-09-08 に修正）。
// 開始を「7ヶ月前」として毎回組み立て、6〜8ヶ月に収まることを見る。
const G6_START = (() => {
  const d = new Date()
...
console.log('=== K4. stripInitialSuffix（氏名を3文字に切らない・#128） ===')
{
  ...
  // 実害(2026-09-17 品質チェック): 管理番号つきの呼称が2文字に切られていた。
  // 営業はその表記で人を指しているので、切ると人を特定できなくなる
  s('K4-14: IT-OGK（ハイフン識別子）は切らない', 'IT-OGK', 'IT-OGK')
  s('K4-15: IC023009（管理番号）は切らない', 'IC023009', 'IC023009')
  s('K4-16: IT-OGK＋年齢性別でも識別子を残す', 'IT-OGK　56歳　男性', 'IT-OGK　56歳　男性')
  // 回帰防止: 数字のあとに更に文字が続くものは従来どおり切る（年齢＋駅名の巻き込み）
  s('K4-17: KM29蕨（年齢＋駅名）はイニシャルのみ', 'KM29蕨', 'KM')
  // 「KM29」は4文字なので、元からある「イニシャル+2文字以内は切らない」規則でそのまま残る。
  // 数字を含む氏名は下流の isUsableName が弾く（apply.mjs）
  s('K4-18: KM29 は長さ規則でそのまま（isUsableName が弾く）', 'KM29', 'KM29')
}

console.log('=== K4b. truncateNameAtBreak（氏名に別項目が混ざる・2026-09-17 名簿チェック） ===')
{
  const { truncateNameAtBreak } = await import('./_extractors.gen.mjs')
  const s = (label, input, expect) => {
    const got = truncateNameAtBreak(input)
    if (got === expect) { pass++; if (verbose) console.log(`  PASS ${label}`) }
    else { fail++; failures.push(label); console.log(`  FAIL ${label}\n       truncateNameAtBreak(${JSON.stringify(input)})=${JSON.stringify(got)} expect=${JSON.stringify(expect)}`) }
  }
  // prod 実測（2026-09-17）: 氏名のあとに別項目が続いて途切れていた（名簿由来）
  s('K4b-1: S.H（男性 → S.H', 'S.H（男性', 'S.H')
  s('K4b-2: M.Y（女性 → M.Y', 'M.Y（女性', 'M.Y')
  s('K4b-3: OY（40歳 → OY', 'OY（40歳', 'OY')
  s('K4b-4: S.R、日本人 → S.R', 'S.R、日本人', 'S.R')
  s('K4b-5: A.B／日本 → A.B', 'A.B／日本', 'A.B')
  // 回帰防止: 括弧が閉じているものは触らない（切るかどうかは stripInitialSuffix の担当）
  s('K4b-6: NK（長野に引っ越し予定）はそのまま', 'NK（長野に引っ越し予定）', 'NK（長野に引っ越し予定）')
  s('K4b-7: tani（38歳・男性）はそのまま', 'tani（38歳・男性）', 'tani（38歳・男性）')
  s('K4b-8: 普通の氏名はそのまま', '山田 太郎', '山田 太郎')
  s('K4b-9: イニシャルはそのまま', 'M.Y', 'M.Y')
}
